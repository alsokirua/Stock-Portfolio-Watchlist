...
def buy():
    ...
    if request.method == "POST":
        ...
        if len(row) == 0:
            ...
            updated_shares = row[0]["shares"] + shares
            db.execute(
...
def history():
    ...
    # Format transactions for display (optional)
    formatted_transactions = []
    for transaction in transactions:
        formatted_transaction = {
            "symbol": transaction["symbol"],
            "shares": transaction["shares"],
            "price": usd(transaction["price"]),  # Formatting price to USD format
            "transacted": transaction[
                "transacted"
            ],  # Assuming 'transacted' is your timestamp column
        }
